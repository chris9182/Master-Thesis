...
In Figure \ref{fig:loaded-data} one can see how the data-view looks when data with cluster labels is shown.
[Figure] loaded data and selector on PCA

In the top right the ``Matrix'' button will open a new window showing the current data in a scatter plot matrix. As the plots on the diagonal of this view are not useful, they were replaced with a kernel density estimation plot.
