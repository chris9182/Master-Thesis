...
\begin{figure}[h]
	...
	\caption{Example Data-Set with unknown Labels \ref{fig:unob}}
	\label{fig:multi}
\end{figure}

Visually, it is already difficult to decide how many clusters can be seen. The points on the left and right can be seen as one or two clusters each, but one could even claim that one side is one and the other two clusters. For this small data-set such claims are easy to be made but with more dimensions such thoughts become more complex. For this reason, I will show how the different results can be found this the tool, by regarding the robustness of the results.
