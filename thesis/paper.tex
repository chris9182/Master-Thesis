...
The aspect of how easily a user can use those often complex clustering and consensus clustering methods was also relevant. There are currently no simple tools that allow to use clustering methods, compare them and compute consensus clusterings from them, all in one place. Therefore, an important consideration was to make this process as easy as possible, while not loosing out on the power the methods bring with them. The strategy here was to clearly separate the pre-processing, the definition of the clustering algorithms and the exploration and merging of the results. To also enable the use of future or currently missing methods, the extendability of the tool was of importance, this is achieved through interfaces with which such methods can easily be added.
