...
\begin{figure}[h]
	\centering
	\includegraphics[scale=.45]{better_base_best}
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=.45]{better_base_consensus}
...
The same strategy as for the example data in Figure \ref{fig:better_base} was tested with the Iris data-set \cite{Dua:2019}. Here Lloyd's k-Means was also used and the parameter $k$ was sampled in the range between $2$ and $10$ computing 10 samples per value $k$. The best result when comparing to the ground truth was for $k=3$ with an NMI of $0.757$ while computing the consensus for all results resulted in a solution with an NMI of $0.797$ (with $k=3$ for the consensus function).
